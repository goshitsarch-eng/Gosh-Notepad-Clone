export function initNotepadApp() {
  ...
  const safeInvoke = (command, payload) => {
    if (!invoke) {
      console.warn(`Tauri invoke not available for ${command}.`);
  ...
  const tauriAPI = {
    newFile: () => safeInvoke('new_file'),
    openFile: () => safeInvoke('open_file'),
    saveFile: (data) => safeInvoke('save_file', { path: data.path, content: data.content }),
    saveFileAs: (data) => safeInvoke('save_file_as', { content: data.content }),
    print: () => safeInvoke('print_document'),
    setTitle: (title) => safeInvoke('set_window_title', { title }),
  ...
  const statusField = statusBar ? statusBar.querySelector('.status-bar-field') : null;

  if (!editor || !statusBar || !statusField) {
    console.warn('Notepad UI not mounted.');
    return;
  }
